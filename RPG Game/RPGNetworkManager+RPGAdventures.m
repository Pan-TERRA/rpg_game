...
static NSString * const kRPGNetworkManagerAPILocationInfoRequestLocationID = @"location_id";

@implementation RPGNetworkManager (RPGAdventures)

#pragma mark - Available Locations
                                                ...
                                                RPGLocationInfoResponse *aResponse))aCallback
{
  NSString *requestString = [NSString stringWithFormat:@"%@%@",
                             kRPGNetworkManagerAPIHost,
                             kRPGNetworkManagerAPIAdventuresLocationInfoRoute];
  
  NSDictionary *requestObject = @{
                                  kRPGNetworkManagerAPILocationInfoRequestLocationID: @(aLocationID)
                                  };
  
  NSURLRequest *request = [self requestWithObject:requestObject
                                        URLstring:requestString
                                           method:@"POST"];
                                                              ...
                                                              NSError * _Nullable error)
  {
    ...
    RPGLocationInfoResponse *responseObject = [[[RPGLocationInfoResponse alloc] initWithDictionaryRepresentation:responseDictionary]
                                               autorelease];
    dispatch_async(dispatch_get_main_queue(), ^
    {
      if (responseObject != nil)
